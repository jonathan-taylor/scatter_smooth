"""
Tests for the C++ natural spline basis implementation.

This module verifies the correctness of the C++ function
`compute_natural_spline_basis` by comparing its output to the equivalent
basis generated by `scipy.interpolate.CubicSpline`. It also tests the
linear extrapolation behavior of the C++ implementation.
"""
import numpy as np
import pytest
...
def test_cpp_basis_matches_scipy():
    """
    Verify that the C++ basis matches the scipy implementation.

    This test generates a natural cubic spline basis using both the C++
    function and `scipy.interpolate.CubicSpline`. It then asserts that
    the resulting basis matrices are all-close.
    """
...
def test_cpp_basis_linear_extrapolation():
    """
    Test the linear extrapolation behavior of the C++ basis.

    This test checks that the spline basis extrapolates linearly outside the
    boundary knots. It does this by comparing the extrapolated values to
    a manual linear projection from the boundary, verifying that the
    second derivative is effectively zero.
    """
...
def test_cpp_basis_random_knots_extrapolation():
    """
    Test linear extrapolation with random knots and a wide range of x.

    This test verifies that the basis functions are linear outside the knot
    boundaries by checking that the second derivative (approximated by the
    second difference) is close to zero.
    """
